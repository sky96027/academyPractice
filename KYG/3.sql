/* 트리거 : 테이블에 대한 이벤트에 반응해 자동으로 실행되는 작업
 ...
 - 트리거 기본 형태
delimiter //
create trigger 트리거명 (after|before) 이벤트명 (intsert|update|delete) on 테이블명
for each row
begin
--실행코드
end //
delimiter;
- old : update 이벤트 발생 시 사용 가능한 것, 값이 바뀌기 전의 행
- new : insert, update 이벤트 발생 시 사용한 것, 값이 바뀐 후의 행 또는 새로 추가된 행
- 변수 방법
...
begin
 ...
 set _amount = new.seamount;
 update fruit
	set
		fr_amout = fr_amout - new.se_amount
	where
